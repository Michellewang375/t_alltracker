...
def forward_video(rgbs, framerate, model, args, global_start=0):
    from utils import saveload, basic, improc
    B, T, C, H, W = rgbs.shape
    device = rgbs.device
    assert C == 3
    # GPU
    grid_xy = basic.gridcloud2d(1, H, W, norm=False, device=device).float()  # 1,H*W,2
    ...
    flows_e = flows_e.to(device)
    traj_maps_e = flows_e + grid_xy  # same device, no error

    # Backward if needed
    if args.query_frame > 0:
        backward_flows_e, backward_visconf_maps_e, _, _ = model.forward_sliding(
            rgbs[:, :args.query_frame+1].flip([1]), iters=args.inference_iters, sw=None, is_training=False
        )
        backward_flows_e = backward_flows_e.to(device)
    ...
    print(f"Processed frames {global_start}-{global_start+T-1} in {ftime:.2f}s")
    ...
    # Draw frames
    frames = draw_pts_gpu(
        rgbs[0], trajs_e[0], visconfs_e[0,:,:,1] > args.conf_thr, colors,
        rate=rate, bkg_opacity=args.bkg_opacity
    )
    # stacking
    if args.vstack:
    ...
    # Save frames with
    for ti in range(T):
        temp_out_f = f'{temp_dir}/{global_start+ti+0:04d}.jpg'
        PIL.Image.fromarray(frames[ti]).save(temp_out_f)
...
def read_image_folder(folder_path, image_size=1024, max_frames=None):
    ...
    img_paths = img_paths[400:694] #694
...
def run(model, args):
    ...
    if args.ckpt_init:
        ...
        import torch.hub
        url = "https://huggingface.co/aharley/alltracker/resolve/main/alltracker_tiny.pth" if args.tiny \
              else "https://huggingface.co/aharley/alltracker/resolve/main/alltracker.pth"
    ...
    # Convert to tensor if it is a list of numpy arrays
    if isinstance(rgbs, list):
        rgbs = [torch.from_numpy(cv2.cvtColor(f, cv2.COLOR_BGR2RGB)) for f in rgbs]  # T,C,H,W
        rgbs = torch.stack(rgbs, dim=0)
    # Ensure batch dimension and float
    if rgbs.ndim == 4:  # T,C,H,W
        rgbs = rgbs.unsqueeze(0)  # 1,T,C,H,W
    B, T, C, H, W = rgbs.shape
    print(f"Loaded {T} frames, shape {rgbs.shape}")
    ...
    if getattr(args, 'mask', None) is not None:
        ...
        mask_tensor = torch.from_numpy((mask_resized > 0).astype(np.float32)).unsqueeze(0).unsqueeze(0)  # 1,1,H,W

    # process-10 per time
    chunk_size = getattr(args, 'chunk_size', 10)
    for start in range(0, T, chunk_size):
        end = min(start + chunk_size, T)
        rgbs_chunk = rgbs[:, start:end].float().cuda()  # Move chunk to GPU
        if mask_tensor is not None:
            rgbs_chunk = rgbs_chunk * mask_tensor.cuda()  # Apply mask

        with torch.no_grad():
            forward_video(rgbs_chunk, framerate, model, args, global_start=start)
